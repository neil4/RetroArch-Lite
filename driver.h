...
#define RARCH_FIRST_LIGHTGUN_BIND RARCH_CUSTOM_BIND_LIST_END
#define RARCH_FIRST_MULTIBUTTON_AREA RARCH_LIGHTGUN_BIND_LIST_END
#define RARCH_FIRST_META_KEY RARCH_MULTIBUTTON_AREA_LIST_END
...
enum
{
   ...
   /* Multibutton areas for overlays */
   RARCH_JOYPAD_DPAD_AREA = RARCH_FIRST_MULTIBUTTON_AREA,
   RARCH_JOYPAD_ABXY_AREA,

   RARCH_MULTIBUTTON_AREA_LIST_END,

   /* Command binds. Not related to game input,
    * only usable for port 0. */
   RARCH_TOGGLE_KEYBOARD_FOCUS = RARCH_FIRST_META_KEY,
   RARCH_ENABLE_HOTKEY,
...
#define MULTIBUTTON_AREA_MASK \
   ( UINT64_C(1) << RARCH_JOYPAD_DPAD_AREA \
   | UINT64_C(1) << RARCH_JOYPAD_ABXY_AREA )

#define META_KEY_MASK \
   ( UINT64_C(1) << RARCH_TOGGLE_KEYBOARD_FOCUS \
   | UINT64_C(1) << RARCH_ENABLE_HOTKEY \
   | UINT64_C(1) << RARCH_FAST_FORWARD_KEY \
   | UINT64_C(1) << RARCH_FAST_FORWARD_HOLD_KEY \
   | UINT64_C(1) << RARCH_LOAD_STATE_KEY \
   | UINT64_C(1) << RARCH_SAVE_STATE_KEY \
   | UINT64_C(1) << RARCH_FULLSCREEN_TOGGLE_KEY \
   | UINT64_C(1) << RARCH_QUIT_KEY \
   | UINT64_C(1) << RARCH_STATE_SLOT_PLUS \
   | UINT64_C(1) << RARCH_STATE_SLOT_MINUS \
   | UINT64_C(1) << RARCH_REWIND \
   | UINT64_C(1) << RARCH_PAUSE_TOGGLE \
   | UINT64_C(1) << RARCH_FRAMEADVANCE \
   | UINT64_C(1) << RARCH_RESET \
   | UINT64_C(1) << RARCH_SHADER_NEXT \
   | UINT64_C(1) << RARCH_SHADER_PREV \
   | UINT64_C(1) << RARCH_CHEAT_INDEX_PLUS \
   | UINT64_C(1) << RARCH_CHEAT_INDEX_MINUS \
   | UINT64_C(1) << RARCH_CHEAT_TOGGLE \
   | UINT64_C(1) << RARCH_SCREENSHOT \
   | UINT64_C(1) << RARCH_MUTE \
   | UINT64_C(1) << RARCH_OSK \
   | UINT64_C(1) << RARCH_NETPLAY_FLIP \
   | UINT64_C(1) << RARCH_SLOWMOTION \
   | UINT64_C(1) << RARCH_VOLUME_UP \
   | UINT64_C(1) << RARCH_VOLUME_DOWN \
   | UINT64_C(1) << RARCH_OVERLAY_NEXT \
   | UINT64_C(1) << RARCH_DISK_EJECT_TOGGLE \
   | UINT64_C(1) << RARCH_DISK_NEXT \
   | UINT64_C(1) << RARCH_DISK_PREV \
   | UINT64_C(1) << RARCH_GRAB_MOUSE_TOGGLE \
   | UINT64_C(1) << RARCH_MENU_TOGGLE )

#define TURBO_ID_ALL RARCH_FIRST_CUSTOM_BIND

#define TURBO_ID_MASK \
   ( UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_B \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_Y \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_A \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_X \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_L \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_R \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_L2 \
   | UINT64_C(1) << RETRO_DEVICE_ID_JOYPAD_R2 )
